...
void adc1_vDeInitFromISR(BaseType_t *const pxHigherPriorityTaskWoken){
	tim3_vDeinitFromISR(pxHigherPriorityTaskWoken);
	adc1_vTakeAccessFromISR(pxHigherPriorityTaskWoken);
	__HAL_RCC_ADC1_CLK_DISABLE();
	HAL_DMA_DeInit(xAdc1.DMA_Handle);
	adc1_vGiveAccessFromISR(pxHigherPriorityTaskWoken);
}
