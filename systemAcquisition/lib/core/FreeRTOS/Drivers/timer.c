...
void tim3_vDeinitFromISR( BaseType_t *const pxHigherPriorityTaskWoken ){

	if(xTim3Semaphore == NULL){
		return;
	}

	xSemaphoreTakeFromISR(xTim3Semaphore, pxHigherPriorityTaskWoken);
	__HAL_RCC_TIM3_CLK_DISABLE();
	HAL_TIM_Base_Stop_IT(&xTim3);
	HAL_NVIC_DisableIRQ(TIM3_IRQn);
	HAL_TIM_Base_DeInit(&xTim3);
	xSemaphoreGiveFromISR(xTim3Semaphore, pxHigherPriorityTaskWoken);
