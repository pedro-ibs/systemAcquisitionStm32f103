 ...
 *  EEPROMemulation.c
 *
 *  Created at:		19/03/2021 12:21:35
 ...
 * @note O bloco deve ter o mesmo tamanho que o buffer da flash sendo 
 * que o buffer tem o tamanho em byte correspondente a uma pagina da
 * memoria flash do micocontrolador no caso do stm32fi03Cx é 1024
 * 
 * @param puCopyTo: ponteiro do bloco de memoria
 */
u8 *flash_puCopyBuffer( u8* puCopyTo ){
	memcpy(puCopyTo, puBuffer, PAGESIZE);
	return puCopyTo;
}
